...
####################################################################################
# This code is a Multi proccesed version of the Monte Carlo Simulation for the SSTA 
# Big Liquid program To run this code more sure to install the requirments
# The data for the simulation can be found in the Outputs folder as
...
def multipro():
    ...
    with multiprocessing.Pool(4) as p:
        ...
        p.join()
        p.close()
        close(outputs, inputs)

def close(output, input):
    output.close()
    input.close()
...
#analysis_parameters, outputs, inputs, inp, out, i = Initialize(4)
#sim(analysis_parameters, outputs, inputs, inp, out, 3)